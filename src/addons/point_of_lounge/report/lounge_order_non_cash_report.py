...
class LoungeOrderDomesticReport(osv.osv):
    _name = "report.lounge.order.non.cash"
    _description = "Lounge Orders Non-Cash"
    _auto = False
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'report_lounge_order_non_cash')
        cr.execute("""
            CREATE OR REPLACE VIEW report_lounge_order_non_cash AS (
                SELECT
                    MIN(lo.id) AS id,
                ...
                WHERE
                    lo.state IN ('paid','invoice') AND aj.type NOT IN ('cash')
                GROUP BY
                    lo.id,rp.id,aj.id
            )
        """)
