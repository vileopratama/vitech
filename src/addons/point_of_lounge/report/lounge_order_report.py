...
class lounge_order_report(osv.osv):
    ...
    _columns = {
        ...
        'state': fields.selection([('draft', 'New'), ('paid', 'Paid'), ('done', 'loungeted'), ('invoiced', 'Invoiced'), ('cancel', 'Cancelled')],
                                  'Status'),
	    ...
	    'total_charge': fields.float('Total Charge', readonly=True),
        ...
        'config_id' : fields.many2one('lounge.config', 'Lounge', readonly=True),
        ...
        cr.execute("""
            create or replace view report_lounge_order as (
                select
                    ...
                    (sum((l.qty * l.price_unit) * (100 - l.discount) / 100) + sum(l.qty * l.charge)) as price_total,
                    sum((l.qty * l.price_unit) * (l.discount / 100)) as total_discount,
                    sum(l.charge * l.qty) as total_charge,
                ...
                from lounge_order_line as l
                    left join lounge_order s on (s.id=l.order_id)
                    left join product_product p on (l.product_id=p.id)
                    left join product_template pt on (p.product_tmpl_id=pt.id)
                    left join product_uom u on (u.id=pt.uom_id)
                    left join lounge_session ps on (s.session_id=ps.id)
                    left join lounge_config pc on (ps.config_id=pc.id)
                group by
                    s.date_order, s.partner_id,s.state, pt.categ_id,
                    s.user_id,s.location_id,s.company_id,s.sale_journal,s.pricelist_id,s.invoice_id,l.product_id,s.create_date,pt.categ_id,pt.lounge_categ_id,p.product_tmpl_id,ps.config_id,pc.stock_location_id
                having
