...
var Gui = core.Class.extend({
    ...
    // goes 1 deep ...
    back: function() {
        var previous = this.lounge.get_order().get_screen_data('previous-screen');
        if (previous) {
    ...
    // This sets up automatic lounge exit when open in
    // another tab.
    close_other_tabs: function() {
        ...
        localStorage['message'] = JSON.stringify({
            'message':'close_tabs',
            'session': this.lounge.lounge_session.id,
        });
    ...
    select_user: function(options){
        ...
        var list = [];
        for (var i = 0; i < this.lounge.users.length; i++) {
            var user = this.lounge.users[i];
        ...
        return def.then(function(user){
            if (options.security && user !== options.current_user && user.lounge_security_pin) {
                return self.ask_password(user.lounge_security_pin).then(function(){
                    return user;
                });
            } else {
                return user;
    ...
    sudo: function(user){
        user = user || this.lounge.get_cashier();
        ...
        if (!pending) {
            ...
            this.lounge.push_order().always(function() {
                ...
                if (!pending) {
                    ...
                    var reason = self.lounge.get('failed') ?
    ...
    play_sound: function(sound) {
        var src = '';
        if (sound === 'error') {
            src = "/point_of_lounge/static/src/sounds/error.wav";
        } else if (sound === 'bell') {
            src = "/point_of_lounge/static/src/sounds/bell.wav";
        } else {
